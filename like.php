...
<body>
    ...
    if ($num_rows > 0) { //unlike button
        echo '<form action="like.php?post_id=' . $post_id . '" method="POST" style="position: absolute; top: 0;">
            <input type="submit" class="comment_like" name="unlike_btn" value="Unlike" " style="background:#1DBF73; padding:3px 10px; color:white;border:none;outline:none">
        ...
        echo '<form action="like.php?post_id=' . $post_id . '" method="POST" style="position: absolute; top: 0;">
                    <input type="submit" class="comment_like" name="like_btn" value="Like" style="background: #1DBF73; border: none; border-radius: 3px; padding: 3px 10px 3px 10px; color: white;">
                    <div class="like_value" style="display: inline;">
                        (' . $total_likes . ')
                    </div>
                </form>
